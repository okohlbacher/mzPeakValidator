...
def _chunk_data(n=3, drop_end=False):
    ...
    drop_end omits mz_chunk_end (to trip chunk_columns)."""
    cols = [pa.array(range(n), pa.uint64()), pa.array([100.] * n, pa.float64()), pa.array([400.] * n, pa.float64()),
    ...
    if drop_end:
        del cols[2]; del names[2]
    chunk = pa.StructArray.from_arrays(cols, names=names)
...
def build_all(out_root):
    ...
    # Phase 3 layout: a chunked facet declaring mz_chunk_start MUST carry mz_chunk_end (chunk_columns).
    case("fail", "chunk_missing_end", _meta(), _chunk_data(drop_end=True), "FAIL", "chunk_columns_spectra_data")
