...
| `spectra_data_footer_implies_rows` | `footer_count_implies_rows` | warning | rederive | Footer {keys} must be 0 when the facet has 0 rows and > 0 when it has rows. The reference writer stamps the run-wide counters on every facet (mzPeakConverter#1: a centroid-only run declares the full spectrum count on an empty spectra_data and a footer-planned reader issued 893,769 queries against it). Advisory until HUPO-PSI defines the footer keys. |
...
| `spectra_data_distinct_count_chunk` | `footer_count_equals_rows` | warning | rederive | Footer spectrum_count equals the number of DISTINCT spectra with rows in THIS file. The reference writer stamps the run-wide ordinal, so a mixed profile/centroid run over-declares on spectra_data (PXD076001: 255,623 declared, 2,521 present). Warning until the converter adopts per-file stamping; not instantiated on spectra_peaks until the zero-peak-spectrum counting question is settled with the converter. |
| `spectra_data_distinct_count_point` | `footer_count_equals_rows` | warning | rederive | Footer spectrum_count equals the number of DISTINCT spectra with rows in THIS file. The reference writer stamps the run-wide ordinal, so a mixed profile/centroid run over-declares on spectra_data (PXD076001: 255,623 declared, 2,521 present). Warning until the converter adopts per-file stamping; not instantiated on spectra_peaks until the zero-peak-spectrum counting question is settled with the converter. |
