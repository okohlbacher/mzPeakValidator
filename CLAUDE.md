...
- **`mzpeak_validator/`** — the installable package (packaging in `pyproject.toml`; `pip install .` ships the profiles as package data; console script `mzpeak-validate = mzpeak_validator.core:main`).
  - **`core.py`** (~950 lines) — the engine: `Archive` (opens a `.mzpeak` ZIP or unpacked dir; exposes Parquet schemas/columns *and* raw ZIP members), `Profile` (loads manifest + rules + column schemas + CV snapshots), `Report` (collates findings), the `PRIMITIVES` dict (20 primitives), and `run()`/`main()`. `PROFILES_ROOT = Path(__file__).parent / "profiles"` resolves the bundled profiles whether running from source or installed.
